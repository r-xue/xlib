pro hastrom_nan,oldim,oldhd,newim,newhd,refhd,MISSING=missing, INTERP = interp, $
               ERRMSG = errmsg,CUBIC = cubic, DEGREE = Degree, NGRID = Ngrid, $
               SILENT = silent

   ;!!!
   ;       This is just a slightly modified hastrom.pro
   ;       20130205  RX    avoid exception when no overlapping regions exist
   ;       20130625  RX    override a problem in extast.pro (extast.pro doesn't work well
   ;                       with fits header from MIRAID in terms of handing free-format and
   ;                       slash (e.g. with output of ctype='RA---SINC /')
   ;       20130904  RX    Epoch precessing (hprecess) or linear transformation (degree=1)
   ;                       doesn't work well with wide-field images having different projections
   ;                       We solve the problems by precessing reference grid points (rather than the image header)
   ;                       and change polywarp degree=2 as default.
   ;!!!
...
;+
 for i=0,n_elements(refhd)-1 do begin
     hdri=strsplit(refhd[i],"'",/extract)
     if n_elements(hdri) ge 2 then begin
         lasthdri=hdri[-1]
         remchar,lasthdri,'/'
         hdri[-1]=lasthdri
         refhd[i]=strjoin(hdri,"'")
     endif
 endfor
;-
...
; Find the position of the reference points in the supplied image
 ...
 ;+
 ; hprecess doesn't work when for a "wide-field" image
 refeq = get_equinox( refhd, code)
 if code EQ -1 then message, $
     ...
     if code EQ -1 then message, $
         'WARNING - Equinox not specified in original header',/CON else $
         if oldeq NE refeq then begin      ;Precess header and re-extract structure
         if (refeq EQ 2000.) and (oldeq EQ 1950.) then begin
             bprecess,ra,dec,ratmp,dectmp
             ra = ratmp & dec = dectmp
             ;print,'refeq2000->oldeq1950'
         endif else if (refeq EQ 1950.) and (oldeq EQ 2000.) then begin
             jprecess,ra,dec,ratmp,dectmp
             ra = ratmp & dec = dectmp
             ;print,'refeq1950->oldeq2000'
         endif else precess, ra, dec, refeq, oldeq                    ;Precess the CRVAL coordinates
     endif
 endelse
 ;-
 ...
 ;+
 sxdelpar,newhd,'EPOCH'
 sxdelpar,newhd,'EQUINOX'
 ;-
